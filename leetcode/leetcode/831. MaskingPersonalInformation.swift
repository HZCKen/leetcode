//
//  831. MaskingPersonalInformation.swift
//  leetcode
//
...
//    12 位数字，2 个数字是国际号码另外 10 个数字是本地号码 。
    ...
    func maskPII(_ S: String) -> String {
        ...
        if S.contains("@") {
            let name1 = String(S.split(separator: "@").first!)
            let name2 = String(S.split(separator: "@").last!)
            let res = String(name1.first!).lowercased() + "*****" + String(name1.last!).lowercased() + "@" + name2.lowercased()
            return res
        } else {
            var res = S
